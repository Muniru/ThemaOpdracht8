...
ggplot() + 
  geom_point(data, mapping = aes(x=time, y=mRNA), colour = "red") + 
  geom_line(out, mapping = aes(x=time, y=mRNA.R)) + 
  geom_line(subset(medians, dose %in% c("0", "0.1")), mapping = aes(x = time, y = mRNA), colour = "blue") +
  geom_line(subset(medians, dose %in% c("0", "0.3")), mapping = aes(x = time, y = mRNA), colour = "green")
```
...
plot_series <- function(y_kon){
  ...
  return(grid.arrange(plot_1, plot_2))
}
